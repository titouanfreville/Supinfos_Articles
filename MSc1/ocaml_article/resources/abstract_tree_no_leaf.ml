(*
    @type : myTree
    Define a simple tree using a match for single value
    V -> empty tree
    N (ln * r * rn) -> Tree with a left node, a root and a right node
*)
type 'a myTree = V | N of ('a myTree * 'a * 'a myTree)
...
let rec myMap tree f =
    match tree with
    | N(fg,r,fd) -> let tmpTree = add (f r) (myMap fg f) in addTreeInTree tmpTree (myMap fd f)
    | _ -> V;;
...
let rec treeDrawing t x y h w printer textColor lineColor=
  ...
  match t with
  | N(V,l,V) -> moveto (x + drawingZoneWidth) y; set_color textColor; printer l; set_color black;
  | N(fg,r,fd) -> begin
